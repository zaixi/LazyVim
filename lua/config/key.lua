...
map.general.t = {
  ["<C-x>"] = { vim.api.nvim_replace_termcodes("<C-\\><C-N>", true, true, true), "   退出终端模式" },
}

map.general.x = {
  -- Don't copy the replaced text after pasting in visual mode
  -- https://vim.fandom.com/wiki/Replace_a_word_with_yanked_text#Alternative_mapping_for_paste
  ["p"] = { 'p:let @+=@0<CR>:let @"=@0<CR>', "不要复制替换文本", opts = { silent = true } },
...
map.general.n = {
  ...
  --['q'] =  { function() require("nvchad_ui.tabufline").close_buffer() end, "   关闭window"},

  ["<leader>c"] = { name = "   按键映射" },
  ["<leader>ck"] = { ":WhichKey <CR>", "WhichKey 查找映射" },
  ["<leader>ch"] = { "<cmd> NvCheatsheet <CR>", "映射 cheatsheet" },
  ...
  ["<leader>tm"] = { "<cmd>MarkdownPreviewToggle <CR>", "切换markdown预览" },
  ...
  -- git {{{
  ["<leader>g"] = { name = "  git" },
  ["<leader>gD"] = { "<cmd>Gitsigns diffthis  <CR>", "当前文件diff" },
  ["<leader>gc"] = { "<cmd>DiffviewClose <CR>", "关闭 Diffview" },
  ["<leader>gd"] = { "<cmd>DiffviewOpen <CR>", "所有文件diff" },
  -- :DiffviewOpen
  -- :DiffviewOpen HEAD~2
  ...
  ["<leader>gB"] = { "<cmd>Git blame <CR>", "所有行 blame" },
  ["<leader>gs"] = { "<cmd>Git <CR>", "git status" },
  ["<leader>gp"] = { "<cmd>Gitsigns prev_hunk<CR>", "上一处更改" },
  ["<leader>gn"] = { "<cmd>Gitsigns next_hunk<CR>", "下一处更改" },
  -- }}}
  ...
  ["<leader>fF"] = {
    ":lua vim.cmd('Telescope find_files find_command=fdfind,--type,f,-p,' .. vim.fn.expand('<cfile>'))  <CR>",
    "查找光标下文件",
  },
  ...
  ["<leader>sm"] = {
    ":lua require'marks'.mark_state:all_to_list('quickfixlist') vim.cmd('Telescope quickfix') <CR>",
    "查找mark",
  },
  -- }}}
  ...
  ["<leader>jJ"] = { ":HopChar1 <CR>", "按一个字符跳" },
  ["<leader>jp"] = { ":HopPattern <CR>", "按Pattern跳" },
  ["<leader>jw"] = { ":HopWord <CR>", "按单词跳" },
